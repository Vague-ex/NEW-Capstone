...
def _temporary_authentication_unavailable_response():
    logger.warning("Transient database error while processing authentication request.", exc_info=True)
    return Response(
        {
            "detail": "Authentication service is temporarily unavailable. Please retry in a few seconds.",
            "retryable": True,
        },
        status=status.HTTP_503_SERVICE_UNAVAILABLE,
    )
...
class AdminLoginView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            return _temporary_authentication_unavailable_response()
...
class AlumniLoginView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            storage_key_basis = (
                alumni_account.master_record.full_name if alumni_account.master_record else alumni_account.user.email
            )
