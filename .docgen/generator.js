...
const questions = [
  ...
  Q("You capture GPS on every login — why isn't it on the map?"),
  A("The login GPS is captured and stored in the login audit trail; that capture already works. The geomap currently visualizes employment and work locations — the core tracer output. Plotting recent-login locations is the same stored coordinates added as a layer, so it's a low-effort next step rather than a new pipeline; for now the login coordinates are retained for audit and security purposes."),
  ...
  A("The data-quality report checks every record against seven required fields — employment status, time-to-hire, sector, job title, work address, technical skills and soft skills — and reports overall coverage, which fields are most often missing, and the completion rate per batch. A record only counts as 'complete' when all seven are present."),
  Q("Why does data quality matter — isn't it trivial?"),
  A("It's the foundation. It pairs with validation at entry: we reject bad data going in and measure coverage after. Everything depends on it — the geomap can't plot a graduate with no work address, and the model shouldn't predict on half-empty records. The report is the integrity check that makes the predictions and the map credible, and demonstrating reliable tracer data is exactly what accreditation expects."),
  Q("What's in the Predictive Employability Trend report?"),
  A("Ten short sections that read top to bottom: an introduction, an executive summary, the observed employment trend per batch (2020–2025), an interpretation, a three-year forecast, notes on the forecast, a model-reliability note with our cross-validated metrics, an observed time-to-hire breakdown, and a conclusion. Observed numbers plus one forward forecast — we don't re-predict batches we already have data for."),
  Q("Why doesn't the report re-predict the past batches?"),
  A("Because we already have their real outcomes; predicting them again only invites confusion. We show what actually happened — the trend — and predict only the next batches, in the forecast section."),
  Q("How is model reliability shown in the report?"),
  A("One plain sentence plus the cross-validated metrics — employment about 90% accurate (0.89 F1 on held-out data), time-to-hire R-squared 0.25. It reads as 'why you can trust the forecast' instead of a table the reader has to decode. We deliberately removed the old actual-vs-predicted table because it confused readers."),
  Q("Can the reports be exported?"),
  A("Yes — every report exports to PDF and Excel; the predictive report's PDF also draws the trend as a line chart."),
  ...
  Q("Why is the time-to-hire forecast floored at one month?"),
  A("A graduate can't realistically be hired in zero months, but a straight-line projection can dip below that — so we floor it at one month to keep the figure sensible."),
  Q("What does the time-to-hire breakdown count — predictions or people?"),
  A("Real people: graduates bucketed by how long they actually took to get hired. Those never hired have no time-to-hire and aren't counted. It's observed data, not a model output."),
  Q("Why did the employment trend look flat before and rise now?"),
  A("Early demo data assigned outcomes randomly, so with only a few graduates per year the line was noisy. We tuned the synthetic generator so later batches are modestly more employable — a realistic upward trend — without letting any batch hit an unrealistic 100%."),
  Q("In one sentence, what does the system do?"),
  A("It verifies graduates with biometrics, tracks and confirms their employment, maps where they work, and turns five years of data into an employability trend and forecast for the BSIS program."),
